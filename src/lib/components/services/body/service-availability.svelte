<script lang="ts">
  ...
  const unavailableColor = dark
    ? "text-service-unavailable-dark"
...
<div id="service-availability" class="mb-s10 flex items-center md:mb-s0">
  {#if service.isAvailable}
    <div
      class={`mr-s8 fill-current ${availableColor} ${dimension} print:text-service-available-dark`}
    >
      {@html emotionHappyIcon}
    </div>

    <span
      class={`${availableColor} print:text-service-available-dark`}
  ...
  {:else}
    <div
      class={`mr-s8 ${dimension} fill-current ${unavailableColor} print:text-service-unavailable-dark`}
    >
